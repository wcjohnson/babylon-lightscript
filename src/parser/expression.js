...
export default class ExpressionParser extends LValParser {
  ...
  parseCallExpressionArguments(close, possibleAsyncArrow, refShorthandDefaultPos) {
    const elts = [];
    let innerParenStart;
    let first = true;
  ...
  parseMethod(node, isGenerator, isAsync) {
    ...
    if (this.hasPlugin("lightscript") && this.match(tt.colon)) {
      ...
      // TODO: support predicates (`%checks`)
      if (node.returnType) node.predicate = null;
  ...
  validateFunctionBody(node, allowExpression) {
