...
const allowUnexpectedKeys = {
  "extra": true,
  "skinny": true,
  "isNowAssign": true,
  "predicate": true,
}
const ignoreEntirelyKeys = {
  "predicate": true,
}
...
function saveLscExp(test, ast) {
  // Ensure that RegExp are serialized as strings
  const toJSON = RegExp.prototype.toJSON;
  RegExp.prototype.toJSON = RegExp.prototype.toString;

  const lscExpectedLoc = test.actual.loc.replace("actual.js", "expected.lightscript.json");
  require("fs").writeFileSync(lscExpectedLoc, JSON.stringify(ast, null, "  "));
...
function misMatch(exp, act) {
  if (exp instanceof RegExp || act instanceof RegExp) {
    ...
    for (var prop in exp) {
      if (ignoreEntirelyKeys[prop]) continue;
    ...
    for (var prop in act) {
      ...
      if (!(prop in exp) && act[prop] !== undefined && !allowUnexpectedKeys[prop]) {
